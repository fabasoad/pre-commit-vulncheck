...
test_log_color_param_precedence() {
  ...
  output=$(PRE_COMMIT_VULNCHECK_LOG_COLOR="${log_color_env_var}" \
    ${SRC_DIR}/main.sh "${command}" \
    "--vulncheck-args=--quiet --hook-args=--log-level=debug --log-color=${log_color_cmd}" \
    2>&1 >/dev/null)
...
test_log_color_env_var() {
  ...
  output=$(PRE_COMMIT_VULNCHECK_LOG_COLOR="${log_color_env_var}" \
    ${SRC_DIR}/main.sh "${command}" \
    "--vulncheck-args=--quiet --hook-args=--log-level=debug" \
    2>&1 >/dev/null)
